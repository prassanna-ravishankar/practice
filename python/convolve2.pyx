...
# The "not None" will enforce that an exception is raised if None is passed in.
# If the "not None" is left out, the function will accept either an ndarray or a None,
# but the None will cause problems later if not explicitly handled.
def naive_convolve(np.ndarray f not None, np.ndarray g not None):
